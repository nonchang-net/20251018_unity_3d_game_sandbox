...
public class GameInputManager : MonoBehaviour, InputSystem_Actions.IPlayerActions
{
    ...
    private Collider characterCollider; // BoxCollider, SphereCollider, CapsuleColliderなど
    ...
    void InitializeCharacter()
    {
        if (targetCharacter == null)
        {
            Debug.LogError("GameInputManager: targetCharacterが設定されていません。");
        ...
        if (animator == null)
        {
            Debug.LogWarning($"GameInputManager: キャラクター '{targetCharacter.name}' にAnimatorが見つかりませんでした。" +
                "アニメーションは再生されません。");
        }
        else if (EnableVerboseLog)
        {
            Debug.Log($"GameInputManager: Animatorを検出しました。({targetCharacter.name})");
        }
        ...
        if (characterController != null)
        {
            ...
            if (EnableVerboseLog)
            {
                Debug.Log($"GameInputManager: CharacterController実装を検出しました。({targetCharacter.name})");
            }
            return;
        }
        ...
        if (characterRigidbody != null && characterCollider != null)
        {
            ...
            if (EnableVerboseLog)
            {
                ...
                Debug.Log($"GameInputManager: Rigidbody+{colliderType}実装を検出しました。({targetCharacter.name})");
            }
        ...
        // どちらの実装も満たさない場合はエラー
        Debug.LogError($"GameInputManager: キャラクター '{targetCharacter.name}' は要件を満たしていません。\n" +
    ...
    void OnHighJump(HighJumpInfo highJumpInfo)
    {
        if (EnableVerboseLog)
        {
            Debug.Log($"GameInputManager: ハイジャンプを発動しました。高さ: {highJumpInfo.JumpHeight}m, 速度: {highJumpInfo.JumpSpeed}");
        }
